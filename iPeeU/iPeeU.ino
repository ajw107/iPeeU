//iPeeU is a brute force iDevice PIN unlocker
//Copyright Alex Wood github@alex-wood.org.uk
//The code can be used by others, and modified,
//but must retain this copyright info
//The hope is that a lot of forensic analysts
//and just people who are drunk/daft and lock
//themselves out of their phone, will find this
//useful.  Also that theives will be too thick
//to know how to use this, but I doubt it.
//But mainly to stop IP-BOX and MFC Dongle from
//an insane price for their equipment.  I know they
//do a lot of work and updates, well the Dongle
//people, not IP-BOX, but the price can not
//be justified for what is a lump of wires and
//a circuit board....
...
//quick function to return the size of an array
...
//now, eventually, the actual program
...
void loop()
{
  ...
  for (int value = START_VALUE; value <= LIMIT; value++)
  {
    ...
    if (doFirst == TRUE)
    {
      //Serial.println("doFirst Active, checking...");
      //only bother if there are any
      if (arr_len(doFirstValues) > 0)
      {
        //Serial.println("Do First Values present");
        for (int i = 0; i < arr_len(doFirstValues); i++)
        {
          //check for match, if so, break
          if (value == doFirstValues[i])
          {
            //Serial.println("Skipping, as found in the list of numbers to do first (so already done");
            breakOut = TRUE;
            break;
          }//if (value == doFirstValues[i])
        }//for (int i=0; i<arr_len(doFirstValues); i++)
    ...
    {
      //Serial.println("Skipping, as found in the range of dates (so already done)!");
      breakOut = TRUE;
    }//if ((value => DATE_RANGE_LOWER) || (value <= DATE_RANGE_UPPER) && (doDate == TRUE))
    ...
    if (breakOut == TRUE)
    {
      //Serial.println("Continue on, nothing to see here");
      continue;
    }//if (breakOut == TRUE)
...
void pressKey(int digit, int pos)
{
  ...
  LED_P8x16Str(10, 0, "Current PIN:");
  itoa(digit, charDigit, 10);
  if ((pos > 0) && (pos < 6))
  {
    LED_P8x16Str((10 * pos + 20), 2, charDigit);
  }

  switch (mode)
  {
    case MODE_DOFIRST: LED_P8x16Str(10, 4, "MODE: Do First"); break;
    case MODE_DATE: LED_P8x16Str(10, 4, "MODE: Date"); break;
    case MODE_NORMAL: LED_P8x16Str(10, 4, "MODE: Normal"); break;
  }
